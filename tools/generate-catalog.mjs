...
function generate(catalog) {
  ...
  validateProvenance(catalog.provenance);
  ...
  out.push(`// Source: ${catalog.provenance.project} ${catalog.provenance.revision}, ${catalog.provenance.file} ${catalog.provenance.database} (${catalog.provenance.vendorVersion}).`);
  out.push("");
  ...
  for (const { name, device } of records) {
    ...
    field(out, "can_erase", String(device.canErase));
...
function validateDevice(device, index) {
  ...
  if (typeof device.canErase !== "boolean") throw new Error(`${label}.canErase must be a boolean`);
  ...
  const flags = parseNumber(device.flagsRaw, `${label}.flagsRaw`);
...
function validateProvenance(provenance) {
  if (!provenance || typeof provenance !== "object") throw new Error("catalog must contain provenance");
  for (const key of ["project", "repository", "revision", "file", "database", "vendorVersion"]) {
    requiredString(provenance, key, "provenance");
  }
}
